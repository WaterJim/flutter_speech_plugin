package com.speech.example.speech_plugin.core;
...
import com.speech.example.speech_plugin.core.listener.IWakeupListener;
...
public class WakeUpHandler {
    ...
    private static final String TAG = "WakeUpHandler";

    public WakeUpHandler(Context context, EventListener eventListener) {
        if (isInited) {
            PluginLogger.error(TAG, "还未调用release()，请勿新建一个新类");
    ...
    public WakeUpHandler(Context context, IWakeupListener eventListener) {
        this(context, new WakeupEventAdapter(eventListener));
    }

    public void start(Map<String, Object> params) {
        String json = new JSONObject(params).toString();
        PluginLogger.info(TAG + ".Debug", "wakeup params(反馈请带上此行日志):" + json);
        wp.send(SpeechConstant.WAKEUP_START, json, null, 0, 0);
    }

    public void stop() {
        PluginLogger.info(TAG, "唤醒结束");
